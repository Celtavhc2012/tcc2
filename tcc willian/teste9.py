...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # Suprime mensagens informativas, warnings, e erros
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Desativa as otimizações oneDNN para evitar mensagens específicas
...
if __name__ == "__main__":
    ...
    # Criação de sequências para a LSTM com tamanho de sequência reduzido para 3
    sequence_length = 3
    sequences = []
    next_events = []
    ...
    # Verificar se y contém rótulos fora do intervalo
    max_label = np.max(y)
    if max_label >= num_classes:
        print(f"Erro: Rótulos em y contêm valores fora do intervalo: {max_label}")
        exit()
