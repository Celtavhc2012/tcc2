...
# Função para criar o modelo LSTM com Input Layer separado e ajuste de hiperparâmetros
def create_lstm_model(input_shape, num_classes):
    model = Sequential()
    model.add(Input(shape=input_shape))
    model.add(LSTM(256, return_sequences=True))  # Aumentando o número de neurônios
    model.add(Dropout(0.5))  # Adicionando mais Dropout para regularização
    model.add(LSTM(128))
    model.add(Dropout(0.5))
    model.add(Dense(64, activation='relu'))
    model.add(Dense(num_classes, activation='softmax'))
    
    # Usando Adam como otimizador com taxa de aprendizado menor
...
if __name__ == "__main__":
    ...
    sequence_length = 5  # Ajustando o tamanho da sequência para captura de contexto
    ...
    lstm_model.fit(X_train, y_train, epochs=50, batch_size=64, validation_data=(X_test, y_test), callbacks=[early_stopping, reduce_lr])
